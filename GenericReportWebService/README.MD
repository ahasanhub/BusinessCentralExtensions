...
https://github.com/hougaard/Youtube-Video-Sources/tree/master/GenericReportService
...
To enable development endpoint to Dynamics Business Central Administrator Shell, follow these steps
1. Check all NAV Server Configuration
```sh
Get-NAVServerConfiguration
```
2.  Run the following commnd to enable SOAPServicesEnabled
```sh
Set-NAVServerConfiguration -KeyName SOAPServicesEnabled -KeyValue true
```
3. Restart NAV Server instance
```sh
...
Report parameter xml from page using report id 111

<?xml version="1.0" standalone="yes"?><ReportParameters name="Customer - Top 10 List" id="111"><Options><Field name="ShowType">0</Field><Field name="NoOfRecordsToPrint">10</Field><Field name="ChartType">0</Field></Options><DataItems><DataItem name="Customer">VERSION(1) SORTING(Field1)</DataItem><DataItem name="Integer">VERSION(1) SORTING(Field1)</DataItem></DataItems></ReportParameters>